...
function ConsignmentForm({ onBack, onPublish, onNavigate, walletAddress }) {
  ...
  const handleFiles = async (files) => {
    ...
    const blobPreviews = fileArr.map(f => URL.createObjectURL(f));
    setForm(prev => ({
      ...prev,
      imagePreview: prev.imageUrls.length === 0 ? blobPreviews[0] : prev.imagePreview,
      imageUrls: [...prev.imageUrls, ...blobPreviews],
    }));
    setUploading(true);
    try {
      const uploaded = [];
      ...
      setForm(prev => {
        ...
        const orderedUrls = [...existingReal, ...uploaded];
        return { ...prev, imageUrls: orderedUrls, imageUrl: orderedUrls[0] || "", imagePreview: orderedUrls[0] || "" };
      });
    } catch (err) {
  ...
  const commission = hammerEstimate * 0.065;
  const buyerPremium = hammerEstimate * 0.15;
  const netPayout = hammerEstimate - commission;
  ...
  const handlePublish = async () => {
    ...
    try {
      await saveAuction({
        ...
        images: form.imageUrls.filter(u => !u.startsWith("blob:")).slice(1),
            ...
            >
              {form.imageUrls.length > 0 ? (
                  ...
                  <div className="cs-img-grid">
                    {form.imageUrls.map((src, idx) => (
                      <div key={idx} className={`cs-img-thumb ${ idx === 0 ? "main" : "" }`}>
                        ...
                        {idx === 0 && <div className="cs-img-main-badge">MAIN</div>}
                ...
                <>
                  ...
                  <div className="cs-upload-hint">Supports JPEG, PNG — select multiple files (Max 50MB each)</div>
                </>
              )}
            ...
            <div className="cs-select-wrap">
              <select className="cs-select" value={form.currency} onChange={e => set("currency", e.target.value)}>
                ...
                <optgroup label="── Crypto">
                  {[{ value: 'BTC', label: 'BTC — Bitcoin ₿' }, { value: 'ETH', label: 'ETH — Ethereum Ξ' }, { value: 'USDT', label: 'USDT — Tether $' }, { value: 'USDC', label: 'USDC — USD Coin $' }, { value: 'BNB', label: 'BNB — BNB' }, { value: 'SOL', label: 'SOL — Solana' }, { value: 'XRP', label: 'XRP — XRP' }, { value: 'ADA', label: 'ADA — Cardano' }, { value: 'DOGE', label: 'DOGE — Dogecoin Ð' }, { value: 'MATIC', label: 'MATIC — Polygon' }].map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
                </optgroup>
              </select>
              <span className="cs-select-arrow">▾</span>
            </div>
            ...
            {openBidNum > 0 ? (
              <>
                <div className="cs-payout-row">
                  ...
                  <span className="cs-payout-row-val">{fmtVal(hammerEstimate)}</span>
                </div>
                <div className="cs-payout-row">
                  <span className="cs-payout-row-label">Buyer's Premium (15%)</span>
                  <span className="cs-payout-row-val" style={{ color: 'var(--accent2)' }}>{fmtVal(buyerPremium)}</span>
                </div>
                <div className="cs-payout-divider">
                  <span className="cs-payout-commission">Platform Commission (6.5%)</span>
                  <span className="cs-payout-commission-val">-{fmtVal(commission)}</span>
                </div>
                <div className="cs-payout-total">
                  <span>YOUR NET PAYOUT</span>
                  <span className="cs-payout-total-val">{fmtVal(netPayout)}</span>
                </div>
                <div className="cs-payout-hint">Based on your opening bid × 1.04 estimate</div>
              </>
            ) : (
    ...
    <div className="al-root">
      ...
      {heroItem && (
        <div className="al-hero">
          {heroItem.image
            ? <img className="al-hero-bg-img" src={heroItem.image} alt="" />
            : <div className="al-hero-bg-empty" />
          }
          ...
          <div className="al-hero-inner">
            <div>
              <div className="al-hero-badges">
                <LiveBadge />
                <span className="al-hero-category">{heroItem.category}</span>
              </div>
              <h1 className="al-hero-title">{heroItem.title}</h1>
              <div className="al-hero-stats">
                <div>
                  <div className="al-hero-stat-label">CURRENT HIGH BID</div>
                  <div className="al-hero-bid">{fmt(heroItem.currentBid, heroItem.currency)}</div>
                </div>
                <div>
                  <div className="al-hero-stat-label">AUCTION ENDS IN</div>
                  <div className="al-hero-timer"><HeroTimer initial={heroItem.initialSeconds || 0} auctionId={heroItem.mongoId} endDate={heroItem.endDate} /></div>
                </div>
              </div>
              <div className="al-hero-btns">
                <button className="al-enter-btn" onClick={() => onQuickJoin(heroItem)}>Enter Live Room</button>
              </div>
            </div>
            {heroItem.image && (
              <div className="al-hero-thumb">
      ...
      <div className="al-footer">
        <span className="al-footer-copy">© 2026 Auction House Live. All rights reserved.</span>
        <span className="al-footer-link">View All Live →</span>
      </div>
      <HamburgerFab onNavigate={onNavigate} />
    </div>
  );
...
function LiveAuctionRoom({ item, onBack, onNavigate, walletAddress }) {
  ...
  const handlePlaceBid = async () => {
    setBidError('');
    if (!bidAmount || bidAmount <= currentHighest) {
      ...
      <div className="room-layout">
        ...
        <div className="room-left">
          ...
          <div className="room-main-img-wrap">
            {images.length > 0
              ? <img key={imgKey} className="room-main-img" src={images[activeImg]} alt="auction item" />
              ...
              <div className="pay-modal" onClick={e => e.stopPropagation()}>
                ...
                {paymentTab === "wallet" && (
                  <div className="pay-tab-body">
                    <div className="pay-qr-placeholder">
                      <img
                        src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${ walletAddress || "auction-wallet" }`}
                        alt="Wallet QR"
                        style={{ width: 150, height: 150, borderRadius: 8, display: "block", margin: "0 auto" }}
                      />
                      <div className="pay-address">{walletAddress || "0x1234...abcd"}</div>
                      <div className="pay-hint">Send exact amount to this wallet address</div>
                    </div>
                ...
                {paymentTab === "upi" && (
                  <div className="pay-tab-body">
                    <div className="pay-qr-placeholder">
                      <img
                        src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=upi://pay?pa=auction@upi"
                        alt="UPI QR"
                        style={{ width: 150, height: 150, borderRadius: 8, display: "block", margin: "0 auto" }}
                      />
                      <div className="pay-address">auction@upi</div>
                      <div className="pay-hint">Scan to pay via any UPI app</div>
                    </div>
                    <div className="pay-amount-row">
    ...
    try {
      ...
      if (auctions?.length > 0) {
        const apiItems = auctions.map(a => {
          ...
          const allImages = [primaryImage, ...extraImages].filter((url, idx, arr) => Boolean(url) && arr.indexOf(url) === idx);
          const now = new Date();
